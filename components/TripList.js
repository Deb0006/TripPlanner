...
export default function TripList(props) {
  ...
  const filteredLessons = lessons.filter((lesson) => {
    ...
    return (
        (lesson.lessonTitle || lesson.tripTitle || '').toLowerCase().includes(searchLower) ||
        (lesson.subject || lesson.destination || '').toLowerCase().includes(searchLower) ||
        (lesson.grade || lesson.specificPlace || '').toLowerCase().includes(searchLower) ||
    ...
    setSingleContent(
        <Card key={lessonData.timestamp}>
          <CardContent sx={{ paddingBottom: 1 }}>
            <Typography variant="h5">{lessonData.lessonTitle || lessonData.tripTitle}</Typography>
            <div
                style={{
                  display: "grid",
                  gridAutoFlow: "column",
            ...
            <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
              ...
              <Tooltip title="Copy to clipboard">
                <IconButton
                    onClick={() => navigator.clipboard.writeText(lessonData.generatedLesson || lessonData.generatedTrip)}
                >
