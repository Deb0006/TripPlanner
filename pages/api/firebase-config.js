...
const firebaseConfigServer = {
  apiKey:
    process.env.FIREBASE_API_KEY || "AIzaSyBH1ZgOp4SFTC5pEWqhPF6Uc_DLwS1ogGM",
...
export async function addData(itemData, userData, docRef) {
  if (docRef) {
    ...
    switch (req.method) {
      ...
      case "PUT":
        ...
        const updatedLessons = putData.generatedLessons.map((lesson) => {
          if (lesson.timestamp.seconds === originalLesson.timestamp.seconds) {
            return {
              ...lesson,
              lessonTitle: newTitle,
              tripTitle: newTitle, // Update both fields for compatibility
            };
          }
          return lesson;
        });
      ...
      case "DELETE":
        ...
        const remainingLessons = deleteData.generatedLessons.filter(
          (lesson) =>
